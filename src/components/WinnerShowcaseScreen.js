...
const WinnerShowcaseScreen = ({ winner, onComplete, prompt }) => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && !Comical && window.Comical) {
      ...
      console.log("Using global ComicalJS from window in WinnerShowcaseScreen");
    }
    ...
    return () => {
      ...
      if (Comical && typeof Comical.stopEditing === 'function') {
        try {
          ...
          console.warn("Error stopping ComicalJS editing in WinnerShowcaseScreen:", e);
        }
      }
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (winner.bubbles && Array.isArray(winner.bubbles)) {
      try {
        console.log(`WinnerShowcaseScreen: Processing ${winner.bubbles.length} bubbles`);
        const createdBubbles = [];
        
        winner.bubbles.forEach((bubble, bubbleIndex) => {
          if (!bubble) return;
          
          console.log(`WinnerShowcaseScreen: Processing bubble ${bubbleIndex}:`, bubble);
          ...
          bubbleEl.id = `winner-bubble-${bubbleIndex}`;
        ...
        setTimeout(() => {
          try {
              ...
              createdBubbles.forEach(({el: bubbleEl, bubble}) => {
                try {
                  ...
                  if (bubble.tailX && bubble.tailY) {
                    ...
                    console.log(`WinnerShowcaseScreen: Using explicit tail position for bubble: ${bubble.tailX}, ${bubble.tailY}`);
                    tailConfig = {
                    ...
                    if (bubble.direction) {
                      ...
                      console.log(`WinnerShowcaseScreen: Using directional tail (${direction}) at position:`, tailConfig.tipX, tailConfig.tipY);
                    }
                  }
                  
                  // Set bubble configuration
                  console.log(`WinnerShowcaseScreen: Setting bubble spec for ${bubbleEl.id}, type=${bubble.type || 'speech'}`);
                  ...
                  console.error("Error configuring individual bubble in WinnerShowcaseScreen:", bubbleError);
                }
              });
              
              // Final update to render all bubbles
              console.log(`WinnerShowcaseScreen: Updating ComicalJS for container with ${createdBubbles.length} bubbles`);
              Comical.update(bubbleContainer);
            } else {
              console.warn("ComicalJS not available for bubble rendering in WinnerShowcaseScreen");
              // Fall back to canvas-based bubble rendering in this case
              winner.bubbles.forEach(bubble => {
                if (!bubble) return;
            ...
            console.error("Error initializing ComicalJS in WinnerShowcaseScreen:", error);
            // Fall back to canvas-based bubble drawing
            winner.bubbles.forEach(bubble => {
              if (!bubble) return;
        ...
        console.error("Error creating bubbles in WinnerShowcaseScreen:", error);
    ...
    return () => {
      ...
      if (Comical && typeof Comical.stopEditing === 'function') {
        try {
          ...
          console.warn("Error stopping ComicalJS editing in WinnerShowcaseScreen cleanup:", e);
        }
      }
    };
  }, [winner, onComplete]);

  // Function to draw a speech or thought bubble (fallback method if ComicalJS fails)
